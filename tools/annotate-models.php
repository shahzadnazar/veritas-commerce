...
foreach ($models as $class => $path) {
    ...
    // Drop every previously generated block so re-running is idempotent.
    // Pint reflows the block after generation — it separates the summary
    // from the annotations with a blank comment line — so the pattern has
    // to tolerate blank lines it did not write.
    // The leading blank comment line is optional: the first generation of a
    // model with no docblock of its own opens with the marker directly, and
    // a regex that insisted on the blank line left that block behind and
    // stacked a second one under it.
    // One block per pass: a single global replace consumes the blank line
    // that separates two stacked blocks, leaving the second one unmatched.
    $stripPattern = '#(?:\n \*)?\n'.preg_quote($marker, '#').'\n(?:(?: \* @property.*| \*)\n)+#';

    do {
        $before = $source;
        $source = (string) preg_replace($stripPattern, "\n", $source, 1);
    } while ($source !== $before);
    ...
    if (preg_match($pattern, $source)) {
        $source = (string) preg_replace_callback(
            ...
            static function (array $m) use ($marker, $block): string {
                ...
                // A docblock stripped back to its opener has nothing to
                // separate from, so it gets no separator line.
                $separator = $doc === '/**' ? '' : " *\n";
