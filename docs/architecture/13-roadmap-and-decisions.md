...
| Payments | **Stripe** behind `PaymentGateway`, **test mode for the pilot** ✅ *Decision 12* | Spec baseline. Connect is the production path when live money starts — it handles the split, seller KYC and automated payouts — but nothing above depends on it | PayPal Commerce, Adyen — all fit the same port |
...
| **M4 · Money** | 7–9 | Seller ledger with the 7-day clearing window, three-figure balance, earnings statement, payout request and admin decision, refunds with reversal at stored rate, commission settings with forward dating and rate history, admin financial screens, reconciliation jobs | All money invariants pass; the admin E2E spec's two assertions pass; a payout request above the *available* balance is rejected |
...
| # | Decision | Owner | Status | Resolution / recommendation | Cost of deferring |
|---|---|---|---|---|---|
| **1** | **Multi-seller order numbering** | Product + Eng | ⏳ Open | One customer-facing number `VC-24081` with per-seller sub-orders `VC-24081-A`. It matches every screen already drawn and is what the data model in [03](03-data-model.md) implements | Cheap now, expensive after launch — the number appears in every email, support conversation and carrier record |
| **2** | **Admin: Filament or a third Inertia area** | Eng + Product | ✅ **Decided 31 Aug 2026** | **Third Inertia area.** The prototype's admin is a designed product, not generic CRUD. Filament would create a second component library that drifts from the design system | Deciding late means rebuilding 15 screens. Decide before M1 |
| **3** | **Tax calculation source** | Finance | ⏳ Open | Flat rate per shipping state for the pilot, stored on the order like every other figure, with `tax_source` recorded. A provider slots behind the same field | Moderate — the checkout UI is identical either way; only the number's origin moves |
| **4** | **Guest checkout account claim** | Product | ⏳ Open | A signed single-use claim link in the confirmation email. Checkout and confirmation already support guests; only the email and a claim route are new | Low — can ship after launch without touching any designed screen |
| **5** | **When does earning become withdrawable** | Finance | ✅ **Decided 31 Aug 2026** | Post to the ledger at **Delivered**, withdrawable after a **7-day clearing window**. Balance renders as three figures: clearing, available, held by an open request | — |
| **6** | **Product review: on by default at launch?** | Product | ⏳ Open | **On for a seller's first 5 listings, then off.** Gets moderation quality without a queue that scales with the catalogue. The settings toggle already allows either | Low — the toggle exists; only the default and the auto-graduation rule are new |
| **7** | **Refund authority** | Product + Ops | ⏳ Open | Admin-only in Phase 1, as the prototype draws it (the seller's order screen says "refunds are handled by the marketplace team") | Low, but must be consistent between the two screens that mention it |
| **8** | **Email templates** | Product + Design | ⏳ Open | The matrix of sends is specified; the artwork is not. Commission a template set against the design system in M5 | Medium — order emails are the most-read surface in the product |
| **9** | **Returns / RMA beyond "Request a return"** | Product | ⏳ Open | Out of Phase 1 scope as written. The button opens a support contact flow; a real RMA workflow is Phase 2 | Low if stated now; high if a customer expects a process that does not exist |
| **10** | **Shipping cost model** | Product + Finance | ⏳ Open | Per-seller flat rate with a free-shipping threshold (both already in store setup and platform settings). Live carrier rates need weights and dimensions — explicitly Phase 1.1 | Low — the fields exist |
| **11** | **Domain, brand assets, legal entity** | Client | ⏳ Open | Needed for M0 (token rename, email sending domain, SPF/DKIM/DMARC) and for the storefront footer and invoices | Blocks email deliverability setup if left late |
| **12** | **Production payment mode at launch** | Client + Finance | ✅ **Decided 31 Aug 2026** | **Test mode for the Phase 1 pilot**, as the specification states. No Stripe Connect KYC, no bank verification and no 1099-K surface in Phase 1; seller onboarding stays the single form the prototype designs | — |
...
## 13.6 Decisions taken

Recorded 31 August 2026. These are settled; the documents in this directory reflect them.

| # | Decision | Outcome | What it changes |
|---|---|---|---|
| **2** | Admin stack | **A third Inertia area**, sharing the same 44-component library — not Filament | M0 builds one component library, not two. All 15 admin screens are built to the prototype exactly. Filament is not a dependency |
| **5** | Earning availability | **Post at Delivered, withdrawable after a 7-day clearing window** | `seller_ledger_entries` carries `available_from`; balance is three figures — clearing, available, held. Payout validation reads available only. See [04](04-money-and-commission.md#42-where-money-moves-step-by-step) |
| **12** | Payment mode at launch | **Stripe test mode for the Phase 1 pilot** | No Connect KYC, no bank verification, no 1099-K in Phase 1. Seller onboarding stays one form. M1 and M4 keep their original size |

## 13.7 What we still need before writing application code

1. **Decision 1 (order numbering)** and **Decision 11 (domain, brand assets, legal entity)** — the first shapes the data model, the second blocks the rename and email authentication setup in week one.
2. Confirmation that the **Phase 1 scope boundary is frozen** (the prototype guide's sign-off checklist).
3. A Stripe test account and a transactional email account.

Decisions 3, 4, 6, 7, 8, 9 and 10 are not blocking: each has a recommendation above and a milestone by which it must land. Work proceeds under the recommendation unless told otherwise.
